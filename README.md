...
下面是一组在 `MetaX` 环境上使用 `DeepSeek-R1-Distill-Qwen-1.5B` 跑出的示例基准结果，对比对象为 `Torch` 基线：
...
| Case | Torch mean(ms) | Torch tok/s | wGInfer mean(ms) | wGInfer tok/s | speedup |
|---|---:|---:|---:|---:|---:|
...
下面是一组在 `NVIDIA` 环境上使用 `DeepSeek-R1-Distill-Qwen-1.5B` 跑出的示例基准结果，对比对象为 `Torch` 基线：
